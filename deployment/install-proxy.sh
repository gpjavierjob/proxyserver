...
usage() {
  ...
  printf "Otros parámetros opcionales son la zona horaria para el servidor Squid, la etiqueta de la\n"
  printf "imagen de docker de Squid a utilizar, un nombre para el contenedor de docker a crear\n"
  printf "y si se desea desplegar el contenedor en MicroK8s.\n"
...
if [ $microk8s -eq $FALSE ]; then
  ...
  if [ $verbose -eq $TRUE ]; then
    echo ""
    docker run -d --name "$container_name" -p 3128:3128 -e TZ="$timezone" -v "$host_snippets_dir:$cont_snippets_dir:ro" -v "$host_logs_dir:$cont_logs_dir" -v "$host_cache_dir:$cont_cache_dir" -v "$host_conf_file:$cont_conf_file:ro" "$image_tag"
    echo -n "... "
  else
    docker run -d --name "$container_name" -p 3128:3128 -e TZ="$timezone" -v "$host_snippets_dir:$cont_snippets_dir:ro" -v "$host_logs_dir:$cont_logs_dir" -v "$host_cache_dir:$cont_cache_dir" -v "$host_conf_file:$cont_conf_file:ro" "$image_tag" > /dev/null
  fi
